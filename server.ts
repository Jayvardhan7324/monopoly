...
interface RoomData {
  ...
  gamePlayerIdToUserId?: Record<string, string>;
  persistedGameId?: string;
}

const MATCH_WIN_REWARD_COINS = 100;
...
function validateStartupEnvironment(isProd: boolean) {
  const fatal: string[] = [];
  ...
  if (!databaseUrl) {
    const issue = 'DATABASE_URL is required so auth, economy, store, and history routes can boot.';
    if (isProd) fatal.push(issue);
    else warnings.push(`${issue} Auth, economy, store, and history routes will be disabled in development.`);
  } else {
    try {
      const parsed = new URL(databaseUrl);
      const authority = databaseUrl.split('://')[1]?.split(/[/?#]/)[0] ?? '';
      const credentials = authority.includes('@') ? authority.slice(0, authority.lastIndexOf('@')) : '';
      if (!/^postgres(ql)?:$/.test(parsed.protocol)) {
        fatal.push('DATABASE_URL must use a postgres:// or postgresql:// URL.');
      }
      if (credentials.includes('@')) {
        fatal.push('DATABASE_URL credentials appear to contain an unencoded "@"; encode it as "%40" or rotate the password.');
      }
    } catch {
      fatal.push('DATABASE_URL is not a valid URL. Check for unencoded special characters in the username/password.');
    }
  }

  if (!authSecret || authSecret.includes('<') || authSecret.length < 32) {
    const issue = 'BETTER_AUTH_SECRET must be a real 32+ character secret, not a placeholder.';
    if (isProd) fatal.push(issue);
    else warnings.push(`${issue} Better Auth will not be production-equivalent in development.`);
  }

  if (isProd) {
    if (!authUrl) {
      fatal.push('BETTER_AUTH_URL is required in production.');
    } else {
      const normalizedAuthUrl = normalizeOrigin(authUrl);
      if (!normalizedAuthUrl) fatal.push('BETTER_AUTH_URL must be a valid absolute URL.');
      if (normalizedAuthUrl?.startsWith('http://')) fatal.push('BETTER_AUTH_URL must use https:// in production.');
    }

    if (!allowedOriginsRaw) {
      fatal.push('ALLOWED_ORIGINS is required in production.');
    } else {
      const origins = allowedOriginsRaw.split(',').map(s => s.trim()).filter(Boolean);
      if (origins.includes('*')) fatal.push('ALLOWED_ORIGINS must not contain "*" in production.');
      for (const origin of origins) {
        const normalized = normalizeOrigin(origin);
        if (!normalized) fatal.push(`ALLOWED_ORIGINS contains an invalid origin: ${origin}`);
        if (normalized?.startsWith('http://') && !normalized.includes('localhost')) {
          fatal.push(`ALLOWED_ORIGINS should use https:// in production: ${origin}`);
        }
      }
    }

    if (!existsSync(path.resolve(process.cwd(), 'dist', 'index.html'))) {
      fatal.push('Production build is missing dist/index.html. Run npm run build before npm start.');
    }
  }
  ...
  if (portRaw) {
    ...
    if (!Number.isInteger(parsedPort) || parsedPort < 1 || parsedPort > 65535) {
      fatal.push('PORT must be an integer between 1 and 65535.');
    }
  }

  if (!process.env.ADMIN_TOKEN || !process.env.ADMIN_USERNAME || !process.env.ADMIN_PASSWORD) {
    warnings.push('Admin panel disabled because ADMIN_TOKEN, ADMIN_USERNAME, or ADMIN_PASSWORD is missing.');
  }
  if (process.env.ADMIN_USERNAME && ['admin', 'root'].includes(process.env.ADMIN_USERNAME.toLowerCase())) {
    warnings.push('ADMIN_USERNAME should not be a generic value like admin/root.');
  }
...
async function startServer() {
  ...
  const startupValidation = validateStartupEnvironment(isProd);
  for (const warning of startupValidation.warnings) {
    console.warn('[STARTUP][WARN]', warning);
  }
  if (startupValidation.fatal.length) {
    console.error('[STARTUP][FATAL] Configuration validation failed:');
    for (const issue of startupValidation.fatal) console.error(` - ${issue}`);
    process.exit(1);
  }
  if (!process.env.ADMIN_TOKEN || !process.env.ADMIN_USERNAME || !process.env.ADMIN_PASSWORD) {
  ...
  if (hasDB) {
    try {
      ...
      if (isProd) {
        console.error("[STARTUP][FATAL] Failed to load Better Auth/DB:", message);
        process.exit(1);
      }
  ...
  function calculateNetWorth(state: any, playerId: number): number {
    const player = state?.players?.find((p: any) => p.id === playerId);
    if (!player) return 0;
    const propertyValue = Array.isArray(state.tiles)
      ? state.tiles
          .filter((tile: any) => tile.ownerId === playerId)
      ...
      : 0;
    return Math.max(0, Number(player.money ?? 0) + propertyValue);
  }

  async function upsertUserStats(tx: any, userId: string, delta: {
    gamesPlayed: number;
    gamesWon: number;
    gamesLost: number;
    ...
    const [existing] = await tx
      .select()
      .from(schema.profilesStats)
      .where(eq(schema.profilesStats.userId, userId));

    if (existing) {
      await tx
        .update(schema.profilesStats)
        .set({
          gamesPlayed: existing.gamesPlayed + delta.gamesPlayed,
          gamesWon: existing.gamesWon + delta.gamesWon,
          gamesLost: existing.gamesLost + delta.gamesLost,
          totalEarnings: existing.totalEarnings + delta.totalEarnings,
          propertiesBought: existing.propertiesBought + delta.propertiesBought,
          peakPropertiesOwned: Math.max(existing.peakPropertiesOwned ?? 0, delta.peakPropertiesOwned),
          bankruptcies: (existing.bankruptcies ?? 0) + delta.bankruptcies,
          totalTurns: (existing.totalTurns ?? 0) + delta.totalTurns,
          updatedAt: new Date(),
        })
        .where(eq(schema.profilesStats.userId, userId));
      return;
    ...
    await tx.insert(schema.profilesStats).values({
      userId,
      gamesPlayed: delta.gamesPlayed,
      gamesWon: delta.gamesWon,
      gamesLost: delta.gamesLost,
      totalEarnings: delta.totalEarnings,
      propertiesBought: delta.propertiesBought,
      peakPropertiesOwned: delta.peakPropertiesOwned,
      bankruptcies: delta.bankruptcies,
      totalTurns: delta.totalTurns,
      updatedAt: new Date(),
    });
  }

  async function recordCompletedGame(roomId: string, room: RoomData, finalState: any) {
    if (!db || !schema?.gameHistory || !schema?.profilesStats || !schema?.profiles || !sql) return;
    if (room.persistedGameId) return;
    if (!finalState || typeof finalState.winnerId !== 'number' || !Array.isArray(finalState.players)) return;

    const historyId = randomUUID();
    room.persistedGameId = historyId;

    try {
      const endedAt = new Date();
      const startedAt = new Date(room.startedAt ?? room.createdAt ?? Date.now());
      const userIdByGamePlayerId = room.gamePlayerIdToUserId ?? {};
      const hostUserId = room.players.find((p: any) => p.id === room.host || p.isHost)?.userId ?? null;
      const winnerUserId = userIdByGamePlayerId[String(finalState.winnerId)] ?? null;

      const finalNetWorth: Record<string, number> = {};
      for (const player of finalState.players) {
        finalNetWorth[String(player.id)] = calculateNetWorth(finalState, player.id);
      }

      const placements = [...finalState.players]
        .sort((a: any, b: any) => (finalNetWorth[String(b.id)] ?? 0) - (finalNetWorth[String(a.id)] ?? 0))
        .map((player: any, index: number) => [String(player.id), index + 1]);
      const placementByPlayerId = Object.fromEntries(placements);

      const historyPlayers = finalState.players.map((player: any) => {
        const ownedCount = Array.isArray(finalState.tiles)
          ? finalState.tiles.filter((tile: any) => tile.ownerId === player.id).length
          : 0;
        return {
          id: player.id,
          userId: userIdByGamePlayerId[String(player.id)] ?? null,
          name: String(player.name ?? 'Player').slice(0, 80),
          isBot: !!player.isBot,
          bankrupt: !!player.isBankrupt,
          finalMoney: Number(player.money ?? 0),
          finalNetWorth: finalNetWorth[String(player.id)] ?? 0,
          ownedProperties: ownedCount,
          placement: placementByPlayerId[String(player.id)] ?? null,
        };
      });

      await db.transaction(async (tx: any) => {
        await tx.insert(schema.gameHistory).values({
          id: historyId,
          roomId,
          hostUserId,
          winnerUserId,
          ...
          turnsPlayed: Math.max(0, Number(finalState.turnCount ?? 0)),
        });

        if (schema.tradeHistory) {
          await tx
            .update(schema.tradeHistory)
            .set({ gameId: historyId })
            .where(and(eq(schema.tradeHistory.roomId, roomId), sql`game_id is null`));
        }

        for (const player of historyPlayers) {
          if (!player.userId) continue;
          await upsertUserStats(tx, player.userId, {
            gamesPlayed: 1,
            gamesWon: player.id === finalState.winnerId ? 1 : 0,
            gamesLost: player.id === finalState.winnerId ? 0 : 1,
            totalEarnings: player.finalNetWorth,
            propertiesBought: player.ownedProperties,
            peakPropertiesOwned: player.ownedProperties,
            bankruptcies: player.bankrupt ? 1 : 0,
            totalTurns: Math.max(0, Number(finalState.turnCount ?? 0)),
          });
        }
        ...
        if (winnerUserId) {
          await tx
            ...
            .set({ coins: sql`coins + ${MATCH_WIN_REWARD_COINS}`, updatedAt: new Date() })
            .where(eq(schema.profiles.id, winnerUserId));
        }
      });
    } catch (err: any) {
      room.persistedGameId = undefined;
      console.error('game_history record failed:', err?.message ?? err);
    }
  }
  ...
  app.post("/api/rooms", async (req, res) => {
    ...
    const player = { id: playerId, originalId: playerId, userId: sessionUser?.id ?? null, name: sanitizeName(data.name), avatar: data.avatar, profileImage: safeImg(data.profileImage), isHost: true };
  ...
  app.post("/api/rooms/random", async (req, res) => {
    ...
    if (targetRoomId) {
      ...
      const player = { id: playerId, originalId: playerId, userId: sessionUser?.id ?? null, name: uniqueName, avatar: data.avatar, profileImage: safeImg2(data.profileImage), isHost: false };
      ...
      const player = { id: playerId, originalId: playerId, userId: sessionUser?.id ?? null, name: sanitizeName(data.name), avatar: data.avatar, profileImage: safeImg3(data.profileImage), isHost: true };
  ...
  app.post("/api/rooms/:id/join", async (req, res) => {
    ...
    if (room.state) {
      ...
      if (allDisconnected) {
        ...
        const player = { id: playerId, originalId: playerId, userId: sessionUser?.id ?? null, name: uniqueName, avatar: data.avatar, profileImage: safeImg4(data.profileImage), isHost: true };
        room.players.push(player);
        ...
        const player = { id: playerId, originalId: playerId, userId: sessionUser?.id ?? null, name: uniqueName, avatar: data.avatar, profileImage: safeImg5(data.profileImage), isHost: false, isSpectator: true };
        room.players.push(player);
    ...
    const player = { id: playerId, originalId: playerId, userId: sessionUser?.id ?? null, name: uniqueName, avatar: data.avatar, profileImage: safeImg6(data.profileImage), isHost: false };
    room.players.push(player);
      ...
      if (roomId) {
        ...
        if (room && room.host === socket.id) {
          ...
          if (data.initialState?.players && Array.isArray(data.initialState.players)) {
            ...
            room.gamePlayerIdToUserId = {};
            for (const rp of room.players) {
              const socketKey = rp.originalId || rp.id;
              const gp = data.initialState.players.find((g: any) => g.name === rp.name);
              if (gp) {
                room.socketToGamePlayerId[socketKey] = gp.id;
                if (typeof rp.userId === 'string' && rp.userId) {
                  room.gamePlayerIdToUserId[String(gp.id)] = rp.userId;
                }
              }
            }
          }
        ...
        if (room && room.host === socket.id) {
          ...
          if (
            ...
            db.insert(schema.tradeHistory).values({
              roomId,
              fromUserId: userIdByGamePlayerId[String(newTradeLog.proposerId)] ?? null,
              toUserId: userIdByGamePlayerId[String(newTradeLog.targetId)] ?? null,
          ...
          const prevWinnerId = typeof room.state?.winnerId === 'number' ? room.state.winnerId : null;
          room.state = data.state; // store full state for reconnects
          if (prevWinnerId === null && typeof data.state?.winnerId === 'number') {
            recordCompletedGame(roomId, room, data.state).catch((err: any) => {
              console.error('game completion persist failed:', err?.message ?? err);
            });
          }
  ...
  app.get('/api/profile/:userId', async (req, res) => {
    try {
      ...
      if (isSelf) {
        const purchases = await db
          .select({ itemId: schema.purchase.itemId, purchasedAt: schema.purchase.purchasedAt })
          .from(schema.purchase)
          .where(eq(schema.purchase.userId, userId));
        const itemIds = purchases.map((p: any) => p.itemId);
        if (itemIds.length) {
          const purchaseByItemId = new Map(purchases.map((p: any) => [p.itemId, p.purchasedAt]));
          const itemRows = await db
            .select()
            .from(schema.storeItem)
            .where(inArray(schema.storeItem.id, itemIds));
          ownedItems = itemRows
            .map((item: any) => ({
              id: item.id,
              name: item.name,
              ...
              active: item.active,
              purchasedAt: purchaseByItemId.get(item.id) ?? null,
              equipped: u.equippedAvatarItemId === item.id,
            }))
            .sort((a: any, b: any) => new Date(b.purchasedAt ?? 0).getTime() - new Date(a.purchasedAt ?? 0).getTime());
        }
      }
      let matchHistory: any[] = [];
      if (schema.gameHistory && desc) {
        const historyRows = await db
          .select()
          .from(schema.gameHistory)
          .where(sql`${schema.gameHistory.players} @> ${JSON.stringify([{ userId }])}::jsonb`)
          .orderBy(desc(schema.gameHistory.endedAt))
          .limit(12);
        matchHistory = historyRows.map((game: any) => {
          const players = Array.isArray(game.players) ? game.players : [];
          const me = players.find((p: any) => p.userId === userId);
          const winner = players.find((p: any) => p.userId === game.winnerUserId)
            ?? players.find((p: any) => p.id === game.winnerUserId)
            ?? players.find((p: any) => p.placement === 1);
          return {
            id: game.id,
            roomId: game.roomId,
            ...
            result: game.winnerUserId === userId ? 'won' : 'lost',
            placement: me?.placement ?? null,
            finalNetWorth: me?.finalNetWorth ?? 0,
