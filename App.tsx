...
const GamePanelFallback = () => (
  <div className="w-full min-h-[260px] p-4">
    <div className="flex h-full min-h-[220px] flex-col gap-3 rounded-2xl border border-slate-800 bg-slate-950/70 p-4">
      <div className="flex items-center gap-3">
        <Skeleton className="size-10 rounded-xl bg-slate-800" />
      ...
      <div className="grid grid-cols-3 gap-2">
        <Skeleton className="h-9 rounded-lg bg-slate-800" />
        <Skeleton className="h-9 rounded-lg bg-slate-800" />
        <Skeleton className="h-9 rounded-lg bg-slate-800" />
      </div>
    </div>
...
const CashlyLoadingScreen = ({ title, subtitle, mode = 'fixed' }: LoadingScreenProps) => (
  ...
  >
    ...
    <div className="relative flex w-full max-w-[360px] flex-col gap-4">
      <div className="flex items-center gap-3">
      ...
      <div className="grid grid-cols-3 gap-2">
        <Skeleton className="h-10 rounded-xl bg-slate-800" />
        <Skeleton className="h-10 rounded-xl bg-slate-800" />
        <Skeleton className="h-10 rounded-xl bg-slate-800" />
      </div>
    </div>
    ...
    <motion.div
      className="hidden w-full max-w-[360px] flex-col items-center gap-5 text-center"
...
const App: React.FC<AppProps> = ({ onOpenStore, onOpenLogin, onOpenProfile, onOpenSettings, onOpenFriends, onSignOut, sessionUser }) => {
  ...
  const selectedBoard = useMemo(() => {
    ...
    return BOARD_PRESETS.find((board: any) => board.name === settings.boardMap)
      ?? savedBoards.find((board: any) => board.name === settings.boardMap)
  ...
  const recordLeaveLoss = () => {
    // Server-side match recording owns stats and rewards.
  };
      ...
      <div className="flex gap-3">
        ...
        <div className="flex-1">
          ...
          {savedBoards.length === 0 && (
            <div className="text-[10px] text-slate-600 mt-1.5">Custom admin boards appear here when saved</div>
          )}
        </div>
      </div>
    </div>
