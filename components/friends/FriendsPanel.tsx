...
import { X, Users, UserPlus, Search, Check, UserX, Loader2 } from 'lucide-react';
        ...
        <AnimatePresence mode="wait">
              ...
              {friends.length === 0 ? (
                ...
                <div key={f.id} className="flex items-center gap-3 bg-slate-800/60 rounded-xl px-3 py-2.5">
                  ...
                  <button
                    ...
                    title="Remove friend"
                  >
                    {acting === f.id ? <Loader2 className="h-4 w-4 animate-spin" /> : <UserX className="h-4 w-4" />}
                  </button>
                </div>
              ))}
            </motion.div>
              ...
              {requests.length === 0 ? (
                ...
                <div key={r.friendshipId} className="flex items-center gap-3 bg-slate-800/60 rounded-xl px-3 py-2.5">
                  ...
                  <button
                    ...
                    title="Accept"
                  >
                    {acting === r.friendshipId ? <Loader2 className="h-4 w-4 animate-spin" /> : <Check className="h-4 w-4" />}
                  </button>
                  <button
          ...
          {tab === 'search' && (
            <motion.div key="search" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-3">
              ...
              {loading && <div className="flex justify-center py-4"><Loader2 className="h-5 w-5 animate-spin text-slate-500" /></div>}
              {searchResults.map(u => (
                <div key={u.id} className="flex items-center gap-3 bg-slate-800/60 rounded-xl px-3 py-2.5">
                  ...
                  >
                    {acting === u.id ? <Loader2 className="h-3 w-3 animate-spin" /> : <UserPlus className="h-3 w-3" />}
                    Add
                  </button>
                </div>
              ))}
