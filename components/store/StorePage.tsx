...
import { ShoppingCart, Coins, Package, Check, ArrowLeft, Sparkles, Tag, Grid3X3, User, Layout, Puzzle, ImageIcon, Zap } from 'lucide-react';
    ...
    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
      {Array.from({ length: 10 }).map((_, i) => (
        <div key={i} className="rounded-xl border border-white/8 bg-white/[0.03] overflow-hidden">
          <Skeleton className="aspect-square rounded-none bg-slate-800" />
...
const StorePage: React.FC<Props> = ({ onBack, userId, onProfilePicEquipped }) => {
  ...
  return (
    <div className="bg-[#080b14] text-white flex flex-col h-full min-h-0">
      ...
      <main className="max-w-5xl mx-auto px-4 py-6 flex-1 overflow-auto w-full flex flex-col gap-6">
        {/* Hero */}
        <div className="relative overflow-hidden rounded-2xl border border-slate-800 bg-[#10151f] p-5 sm:p-6 shadow-xl shadow-black/20">
          ...
          <div className="relative z-10 flex items-center justify-between gap-4 flex-wrap">
            <div>
              <div className="flex items-center gap-2 mb-1.5">
                ...
                <span className="text-[10px] font-black uppercase tracking-[0.22em] text-amber-200">Cashly Store</span>
              </div>
              <h2 className="text-2xl font-black tracking-tight mb-1 text-white">Customize Your Empire</h2>
              <p className="text-slate-400 text-xs sm:text-sm">Spend match coins on avatars, profile pictures, board skins, and table tokens.</p>
            </div>
                  ...
                  >
                    ...
                    <div className="p-2.5">
                      ...
                      <div className="mt-1.5 flex items-center gap-1">
                        ...
                        {busy && <span className="ml-auto text-[10px] font-black text-slate-400">...</span>}
                      </div>
                    </div>
                  </motion.div>
                );
