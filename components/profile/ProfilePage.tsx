...
import {
  ...
  Pencil, Check, X, Users, Star, ArrowLeft,
  Package, Clock, Camera, Crown,
} from 'lucide-react';
...
function ProfileSectionSkeleton() {
  return (
    <div className="bg-[#1a1a24] border border-white/5 rounded-2xl p-5 flex flex-col gap-3">
    ...
    <div className="w-full h-full flex flex-col bg-[#13131a] overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto scrollbar-thin scrollbar-thumb-slate-800 scrollbar-track-transparent">
        <div className="max-w-3xl mx-auto px-5 py-8 flex flex-col gap-8">
              ...
              {editing ? (
                <div className="flex items-center gap-2 mb-1 justify-center sm:justify-start">
                  ...
                  <button onClick={saveName} disabled={saving} className="p-1.5 text-emerald-400 hover:bg-emerald-500/20 rounded-lg transition-colors shrink-0">
                    <Check className={`h-4 w-4 ${saving ? 'opacity-50' : ''}`} />
                  </button>
          ...
          <section>
            ...
            {loading ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                {[0, 1, 2, 3].map(i => <Skeleton key={i} className="h-20 rounded-xl bg-slate-800" />)}
              </div>
            ) : (profile?.ownedItems?.length ?? 0) > 0 ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                {profile!.ownedItems!.map(item => (
                  <div key={item.id} className="bg-[#1a1a24] border border-white/5 rounded-xl px-4 py-3 flex items-center gap-3 min-w-0">
                    <div className="size-11 rounded-xl bg-slate-800 border border-white/5 flex items-center justify-center overflow-hidden shrink-0">
                      {item.assetUrl ? (
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2 min-w-0">
                        ...
                        {item.equipped && <Crown className="h-3.5 w-3.5 text-amber-400 shrink-0" />}
                      </div>
                      <p className="text-[11px] text-slate-500 capitalize truncate">{item.type.replace('_', ' ')}</p>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="bg-[#1a1a24] border border-white/5 rounded-2xl px-5 py-10 flex flex-col items-center gap-2">
                ...
                <p className="text-sm text-slate-600">No owned items yet</p>
              </div>
            )}
          </section>
          ...
          {/* ── Last games ─────────────────────────────────────── */}
          <section>
            <h2 className="text-base font-black text-white mb-4">Last games</h2>
            {loading ? (
              <ProfileSectionSkeleton />
            ) : (profile?.matchHistory?.length ?? 0) > 0 ? (
              <div className="bg-[#1a1a24] border border-white/5 rounded-2xl divide-y divide-white/5 overflow-hidden">
                {profile!.matchHistory!.map(game => (
                  <div key={game.id} className="px-5 py-4 flex items-center gap-3">
                    <div className={`size-10 rounded-xl flex items-center justify-center shrink-0 ${game.result === 'won' ? 'bg-emerald-500/15 text-emerald-300' : 'bg-slate-800 text-slate-400'}`}>
                      {game.result === 'won' ? <Trophy className="h-4 w-4" /> : <Clock className="h-4 w-4" />}
                    </div>
                    <div className="min-w-0 flex-1">
                      <p className="text-sm font-bold text-white truncate">
                        {game.result === 'won' ? 'Victory' : `Winner: ${game.winnerName}`}
                      </p>
                      <p className="text-[11px] text-slate-500">
                        {new Date(game.endedAt).toLocaleDateString()} - {game.turnsPlayed} turns - {game.playerCount} players
                      </p>
                    </div>
                    <div className="text-right shrink-0">
                      <p className="text-sm font-black text-white tabular-nums">${game.finalNetWorth.toLocaleString()}</p>
                      <p className="text-[10px] text-slate-500">#{game.placement ?? '-'}</p>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="bg-[#1a1a24] border border-white/5 rounded-2xl px-5 py-10 flex flex-col items-center gap-2">
                ...
                <p className="text-sm text-slate-600">No completed matches yet</p>
              </div>
            )}
          </section>
