...
import { 
  Dices, ArrowRight, XCircle, CheckCircle, MapPin, Trophy, Landmark, 
  Handshake, Search, Coins, X, Info, TrendingUp, ChevronDown, ChevronUp,
  Gavel, Timer, Hammer
} from 'lucide-react';
...
export const Controls: React.FC<ControlsProps> = ({ gameState, onRoll, onBuy, onEndTurn, onUpgrade, onOpenProperty, onTrade, dispatch }) => {
  ...
  const myProperties = useMemo(() => 
    gameState.tiles.filter(t => t.ownerId === 0 && t.buildingCount === 0), 
    [gameState.tiles]
  );

  const targetPlayerProperties = useMemo(() => 
    targetPlayerId !== null ? gameState.tiles.filter(t => t.ownerId === targetPlayerId && t.buildingCount === 0) : [],
    [gameState.tiles, targetPlayerId]
  );

  // Local state for dice animation to allow "reveal" time after rolling stops
  const [isRollingAnim, setIsRollingAnim] = useState(false);

  useEffect(() => {
      if (gameState.phase === 'MOVING') {
          setIsRollingAnim(true);
          // Dice spin for 0.5s, then stop to show result while phase continues
    ...
    <div className="w-full h-full flex flex-col gap-4 text-slate-100 p-2 animate-fade-in relative">
      ...
      <div className="grid grid-cols-[1fr_auto] gap-2 md:gap-4">
        <div className="bg-slate-900/40 backdrop-blur-xl p-3 md:p-4 rounded-2xl flex items-center justify-between relative overflow-hidden border border-white/10 shadow-2xl">
          ...
          {/* Trade Button - Always Visible with Shimmer Animation */}
          <button 
      ...
      <div className="flex-1 flex flex-col gap-4 relative min-h-0">
        {/* Auction Overlay - Covers everything below header */}
            ...
            <div className="absolute inset-0 z-50 bg-[#0f172a]/95 backdrop-blur-md rounded-2xl border border-indigo-500/30 shadow-[0_0_100px_rgba(79,70,229,0.4)] animate-fade-in flex flex-col items-center justify-center p-8 overflow-hidden">
                 ...
                 <div className="flex flex-col items-center gap-4 mb-12">
                    ...
                    {gameState.auction.highestBidderId !== null && (
                        <div className={`flex items-center gap-3 mt-6 px-6 py-3 rounded-2xl border transition-all duration-300 ${gameState.auction.highestBidderId === 0 ? 'bg-emerald-500/10 border-emerald-500/50 shadow-[0_0_20px_rgba(16,185,129,0.1)]' : 'bg-slate-800/80 border-white/10'}`}>
                 ...
                 <div className="w-full max-w-md flex gap-6 mb-12">
                    <button 
                        onClick={() => dispatch({ type: 'PLACE_BID', payload: { playerId: 0, amount: (gameState.auction?.currentBid || 0) + 10 } })}
                        disabled={gameState.players[0].money < (gameState.auction?.currentBid || 0) + 10 || gameState.players[0].isBankrupt}
                    ...
                    >
                        ...
                        <span className="flex items-center gap-2">+$10</span>
                    </button>
                    <button 
                    ...
                    >
                        ...
                        <span className="flex items-center gap-2">+$100</span>
                    </button>
                 </div>
                 ...
                 <div className="w-full max-w-md flex flex-col gap-3 z-10">
                    ...
                    <div className="w-full bg-slate-800/50 rounded-full h-3 overflow-hidden border border-white/5">
                        <div 
                            ...
                            style={{ width: `${(gameState.auction.timer / 10) * 100}%` }}
                        />
                    </div>
                 </div>
            </div>
        ...
        {/* Main Center Action Area / Trade UI */}
              ...
              <div className="bg-[#0f172a] p-4 flex flex-col gap-4 overflow-hidden">
                ...
                <div className="flex-1 flex flex-col gap-2 overflow-hidden">
                  ...
                  <div className="flex-1 overflow-y-auto pr-1 space-y-1 scrollbar-thin scrollbar-thumb-slate-800">
                    ...
                    {myProperties.length === 0 && <div className="text-center py-8 text-slate-600 text-xs italic">No tradeable assets</div>}
                  </div>
                </div>
              </div>
              ...
              <div className="bg-[#0f172a] p-4 flex flex-col gap-4 overflow-hidden">
                ...
                <div className="flex-1 flex flex-col gap-2 overflow-hidden mt-2">
                  <div className="flex-1 overflow-y-auto pr-1 space-y-1 scrollbar-thin scrollbar-thumb-slate-800">
                    ...
                    {targetPlayerProperties.length === 0 && <div className="text-center py-8 text-slate-600 text-xs italic">No tradeable assets</div>}
                  </div>
                </div>

                <div className="bg-slate-900/50 p-3 rounded-xl border border-white/5 space-y-2 mt-auto">
                  <div className="flex justify-between text-[10px] font-bold uppercase tracking-wider text-slate-500">
                    <span>Summary</span>
                    <span className="text-slate-400">Total: ${offeredCash + selectedOfferPropertyIds.reduce((sum, id) => sum + gameState.tiles[id].price, 0)}</span>
                  </div>
                  <button 
          ...
          <>
            {gameState.winnerId !== null ? (
                <div className="flex flex-col items-center gap-4 animate-slide-up text-center">
                    ...
                    <p className="text-slate-400 text-sm font-bold uppercase tracking-widest">{gameState.players.find(p => p.id === gameState.winnerId)?.name} is the last one standing</p>
                    <button onClick={() => window.location.reload()} className="mt-6 px-10 py-4 bg-white text-black rounded-xl font-black text-lg hover:bg-slate-200 transition-all uppercase tracking-tight">New Empire</button>
                </div>
            ) : (
              <>
                            ...
                            currentPlayer.inJail ? (
                                <div className="flex flex-col items-center gap-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
                                    <div className="flex items-center gap-2 px-4 py-2 bg-rose-500/10 border border-rose-500/30 rounded-full text-rose-400 text-[10px] font-black uppercase tracking-[0.2em]">
                                        <Lock size={14} /> Currently in Jail (Turn {currentPlayer.jailTurns + 1}/3)
                                    </div>
                     ...
                     >
                        <div className="w-full bg-black/90 rounded-2xl p-8 border border-white/10 shadow-[0_20px_50px_rgba(0,0,0,0.5)] text-center">
                            ...
                            {!currentPlayer.isBot && (
                                ...
                                >
                                    {gameState.phase === 'ACTION' ? (gameState.settings.rules.auctionEnabled ? 'PUT TO AUCTION' : 'SKIP TRANSACTION') : 'FINISH TURN'} 
        ...
        {/* Footer Area - Protocol Feed expanded */}
            ...
            <div className="flex-1 bg-[#0f172a]/60 rounded-xl flex flex-col p-4 border border-white/5 overflow-hidden backdrop-blur-sm">
                ...
                <div className="flex-1 overflow-y-auto space-y-2 pr-2 scrollbar-thin scrollbar-thumb-slate-700">
                    {gameState.logs.map((log, i) => (
                        <div key={i} className={`text-[11px] font-bold leading-relaxed transition-opacity duration-500 ${i===0 ? 'text-indigo-300 border-l-2 border-indigo-500 pl-3 animate-pulse' : 'text-slate-500 pl-3 opacity-60'}`}>
                            {log}
                        </div>
                    ))}
                </div>
