    ...
    >
      ...
      >
        ...
        <div className={`${colorMap[tile.group]} p-6 relative overflow-hidden`}>
          ...
          <button onClick={handleClose} className="absolute top-4 right-4 text-white/50 hover:text-white transition-colors z-10">
            <X size={20} />
          </button>
        ...
        <div className="p-6">
            ...
            <div className="space-y-1">
              ...
              {owner ? (
                <div className="flex items-center gap-2">
                  <Avatar avatarId={owner.avatar} color={owner.color} isBankrupt={owner.isBankrupt} inJail={owner.inJail} className="w-5 h-5" />
            ...
            {!tile.isMortgaged ? (
              <div className="text-xs flex flex-col gap-1.5">
                ...
                {isProperty && [1, 2, 3, 4, 5].map(lvl => (
                  <div
                    key={lvl}
                    className={`flex justify-between items-center px-1 rounded ${tile.buildingCount === lvl ? 'text-white font-bold bg-white/5 py-0.5' : 'text-slate-400'} ${lvl === 5 ? 'mt-1 pt-1 border-t border-white/5' : ''}`}
                  >
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              {/* IMP-19: Sell confirmation — two-tap to confirm */}
                ...
                <AnimatePresence mode="wait">
                  {showSellConfirm ? (
                    ...
                    >
                      <button
                        onClick={handleSell}
                        className="flex-1 bg-rose-600 hover:bg-rose-500 text-white border border-rose-500 py-2.5 rounded-xl text-xs font-bold flex items-center justify-center gap-1 transition-all active:scale-95"
                      >
                        <AlertTriangle size={12} /> Confirm
                      </button>
                      <button
                        onClick={() => setShowSellConfirm(false)}
                        className="flex-1 bg-slate-800 hover:bg-slate-700 text-slate-300 border border-slate-700 py-2.5 rounded-xl text-xs font-bold transition-all active:scale-95"
                      >
                        Cancel
                      </button>
                    </motion.div>
                    ...
                    >
                      {/* BUG-09: Fixed label from "Sell Bank" to "Sell to Bank" */}
                      <Landmark size={14} /> Sell to Bank (+${sellValue})
                    </motion.button>
                  )}
                </AnimatePresence>
              )}
