  ...
  switch (action.type) {
    ...
    case 'LAND_ON_TILE': {
      ...
      if (tile.type === TileType.CORNER) {
        ...
        if (tile.name === 'Vacation' || tile.name === 'Free Parking') {
          ...
          if (state.settings.rules.vacationCash && state.taxPool > 0) {
            ...
            logs = addLog(logs, `${player.name} landed on ${tile.name}, collected the $${winAmount} pool, and will rest next turn!`);
            return withSound({ ...state, players: newPlayers, taxPool: 0, logs, phase: 'TURN_END', lastDiceRollDoubles: false, doublesCount: 0 }, 'buy');
          }
          logs = addLog(logs, `${player.name} is on ${tile.name} and will skip their next turn!`);
