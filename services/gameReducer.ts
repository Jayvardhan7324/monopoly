...
export type Action =
  ...
  | { type: 'PASS_AUCTION'; payload: { playerId: number; senderPlayerId?: number } }
...
const coreReducer = (state: GameState, action: Action): GameState => {
  switch (action.type) {
    ...
    case 'START_GAME': {
      ...
      const reserveRandomAvatar = (): number => {
        const available = APPEARANCE_COLORS
          .map((_, index) => index)
          .filter(index => !usedAvatarIds.has(index));
        const chosen = available.length > 0
          ? available[Math.floor(Math.random() * available.length)]
          : Math.floor(Math.random() * APPEARANCE_COLORS.length);
        usedAvatarIds.add(chosen);
        return chosen;
      };
      ...
      if (lobbyPlayers && lobbyPlayers.length > 0) {
        ...
        players = lobbyPlayers
          ...
          .map((p: any) => {
            const avatarId = p.isBot ? reserveRandomAvatar() : reserveAvatar(p.avatar ?? 0);
            return {
      ...
      for (let i = 0; i < botCount; i++) {
        ...
        const avatarId = reserveRandomAvatar();
        players.push({
          id: botId,
          name: genBotName(),
          color: colorFromAvatar(avatarId),
    ...
    // ─── PASS_AUCTION ─────────────────────────────────────────────────────────
    case 'PASS_AUCTION': {
      if (state.winnerId !== null || !state.auction) return state;
      const { playerId } = action.payload;
      if (!senderMatches(action, playerId)) return state;
      if (state.auction.highestBidderId === playerId) return state;
      if (!state.auction.bidders.includes(playerId)) return state;

      const player = state.players.find(p => p.id === playerId);
      const nextBidders = state.auction.bidders.filter(id => id !== playerId);
      const nextAuction = { ...state.auction, bidders: nextBidders };
      const tileName = state.tiles[state.auction.tileId]?.name ?? 'the property';
      const nextLogs = addLog(state.logs, `${player?.name ?? 'Player'} passed on ${tileName}.`);
      const hasWinner = nextAuction.highestBidderId !== null;
      const challengersLeft = nextBidders.some(id => id !== nextAuction.highestBidderId);
      ...
      return withSound(
        {
          ...
          auction: {
            ...nextAuction,
            timer: hasWinner && !challengersLeft ? 0 : nextAuction.timer,
          },
          logs: nextLogs,
        },
        'modal_close'
